...
Value fcenable(const Array& params, bool fHelp)
{
    ...
    if (fSecMsgEnabled)
        throw runtime_error("Fibre Connect© is already enabled.");
    
    Object result;
    if (!SecureMsgEnable())
    {
        result.push_back(Pair("result", "Failed to enable Fibre Connect©."));
    } else
    {
        result.push_back(Pair("result", "Enabled Fibre Connect©."));
    }
    return result;
}

Value fcdisable(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "fcdisable \n"
    ...
    if (!fSecMsgEnabled)
        throw runtime_error("Fibre Connect© is already disabled.");
    
    Object result;
    if (!SecureMsgDisable())
    {
        result.push_back(Pair("result", "Failed to disable Fibre Connect©."));
    } else
    {
        result.push_back(Pair("result", "Disabled Fibre Connect©."));
    }
    return result;
}

Value fcoptions(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            "fcoptions [list|set <optname> <value>]\n"
...
Value fclocalkeys(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            "fclocalkeys [whitelist|all|wallet|recv <+/-> <address>|anon <+/-> <address>]\n"
    ...
    {
        result.push_back(Pair("result", "Unknown Mode."));
        result.push_back(Pair("expected", "fclocalkeys [whitelist|all|wallet|recv <+/-> <address>|anon <+/-> <address>]"));
    };
    
    return result;
};

Value fcscanchain(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "fcscanchain \n"
...
Value fcscanbuckets(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "fcscanbuckets \n"
...
Value fcaddkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "fcaddkey <address> <pubkey>\n"
...
Value fcgetpubkey(const Array& params, bool fHelp)
{
...
Value fcsend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "fcsend <addrFrom> <addrTo> <message>\n"
            "Send an secure message from addrFrom to addrTo.");
...
Value srvnotify(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "Sends an service message to addrTo");
...
Value fcinbox(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1) // defaults to read
        throw runtime_error(
            "fcinbox [all|unread|clear]\n"
...
Value fcoutbox(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1) // defaults to read
        throw runtime_error(
            "fcoutbox [all|clear]\n"
...
Value fcbuckets(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "fcbuckets [stats|dump]\n"
